...
### Dense regen + silhouette (back#53)

The 3 fixtures were regenerated by the front's dense masked-board tooling
(front task 7, `MazePruebaFront/tool/level_production/themed/`) and copied
1:1 into this repo — only `levelId` (`themed-<name>` → `t-<name>`) and
`section: "themed"` change; `cols`, `rows`, `palette`, `arrows` and the new
`silhouette` field are byte-identical to the front source. Verified
programmatically (deep-equal on both sides, one-off script, deleted before
commit) — see the task 11 report for the raw diff output.

`silhouette` (`Record<role, [row, col][]>`) is the full fill of each paint
region's mask, opaque to the domain (`LevelSilhouette` carrier, back#53) and
checked only for basic shape by `validateLevelSilhouette` (no visual
semantics) in `prisma/seed.ts`'s `validate()` step, right after
`validateLevelPaint`. It travels inside the JSON `data` column — no schema
migration.

t-heart and t-happy-face were regenerated at much higher arrow density (the
production tooling picks the seed maximizing coverage of the silhouette
union, mirrored by a ≥0.90 density guardian in both repos' test suites).
**t-bunny is frozen**: its 37 arrows are untouched byte-for-byte from the
prior regen (back#47) — it only gained the `silhouette` key.

| level | source | dims (cols x rows) | arrows | roles | coverage (arrow cells / silhouette union) |
|---|---|---|---|---|---|
| t-heart | front tooling, dense regen with silhouette (back#53) — mask `heart`, seed 67 | 36x24 | 190 | heart | 0.9885 |
| t-happy-face | front tooling, dense regen with silhouette (back#53) — mask `happy_face`, seed 41 | 24x22 | 129 | face, features | 0.9796 |
| t-bunny | front themed tooling — mask `bunny`, seed 11 — **FROZEN**, arrows untouched since back#47, silhouette-only addition (back#53) | 16x20 | 37 | fur, pink, eye | 0.8721 (not guarded — frozen fixture) |
...
`curated-levels.spec.ts` (LevelSolver + paint consistency + silhouette
consistency + density guardian).
